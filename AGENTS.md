...
- Route every visible Settings destination change through `SettingsDestinationSelection`. It owns the explicit non-animated selection transaction for both sidebar and programmatic routes; future `SettingsTab` cases automatically use it through the sidebar `ForEach`. Do not write `selectedSettingsTab` directly from a new visible navigation control or route.
