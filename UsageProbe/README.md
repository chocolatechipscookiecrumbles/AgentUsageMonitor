...
The native companion has a separate macOS Settings window with General, Notifications, Refresh, Agents, Data & Privacy, and Diagnostics tabs. Notifications contains the working permission and category controls; Refresh includes the real manual-refresh action. Agents identifies OpenAI Codex as the only active integration and lists Claude Code and GitHub Copilot as planned and not connected. Other tabs report current app, collection, retention, and classified-outcome status without exposing fingerprints or raw errors. Adaptive interval choices, agent connection flows, launch at login, export, and deletion remain later work. Remaining-quota thresholds stay fixed at 50%, 25%, 10%, and 5% for both the five-hour and weekly lanes; forecast and reset-credit-expiration warnings can be disabled independently. These preferences affect only the native app and do not change the Python probe.
