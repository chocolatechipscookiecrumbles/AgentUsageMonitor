...
The former separate Dashboard-window plan was replaced on 2026-07-28 by a **Token Monitor** card in each provider tab of the existing menu popover, now implemented. Each agent's Settings page controls whether its card appears and which of its four sections are shown; hiding an agent's card stops reading that agent's records, purges its persisted requests, and resets decoded source state so re-enable scans from scratch. While the app runs, it automatically performs field-scoped, zero-token-cost reads of known provider session roots. Recent reconciled totals are cached in the app's own folder so a launch republishes them before rescanning; only displayed figures are cached, never paths, agent session identifiers, or record contents. The dynamic-height **This Mac · observed** card uses hoverable 30-minute completion-time bars and lists provider-native token rows, Requests, the top three short model groups plus counted Other, and Last Request across all observed dates. It distinguishes missing records, unsafe reads, and a valid zero day, remains independent of quota availability, and selectively decodes usage metadata without retaining or transmitting conversation content.
