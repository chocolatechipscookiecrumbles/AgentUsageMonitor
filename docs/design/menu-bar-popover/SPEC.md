...
- **Tab hit target is the full column and taller** — strip height `36 → 44px`, with a full-column hover fill so the target reads as large as it is. The fill frame and `contentShape` must sit **inside** the `Button` label so the plain button owns the whole column; applied outside the button, only the text glyphs stayed clickable (the 2026-07-24 hit-area fix). The Settings Agents strip carries the same contract — each fixed-size tab declares `.contentShape(.rect)` so the whole tab, not just its icon and text, is the target.
