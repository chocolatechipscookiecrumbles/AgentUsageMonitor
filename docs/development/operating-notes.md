...
Data & Privacy also acts on that storage. **Reveal in Finder** and **Copy Path** open or copy the Application Support folder, falling back to its parent before the first refresh writes anything. **Export Local Data…** writes every listed store to one JSON file you choose, naming and marking as unavailable any store this Mac has not written rather than omitting it. Inventory files are opened descriptor-relatively without following symbolic links, so a planted symlink cannot widen the export beyond the app-owned folder; Claude Code's Keychain item and the agents' own records are never exported. The exported file leaves the app's owner-only folder and is protected only by where you save it. Deletion controls remain deferred — remove the files in Finder to clear this data today.
...
Each agent's Settings page has a **Token Monitor** section: one **Show token monitor** toggle, a **Range** choice of **Day** or **Week**, and the **Activity chart**, **Token categories**, **Model usage**, and **Last request** section toggles. Turning an agent's Token Monitor off stops reading that agent's records, removes its persisted requests, and resets decoded per-file source state. Re-enabling therefore scans from scratch, and an interrupted older scan cannot publish after the transition. It is a collection control rather than a display filter. The card's title, range line, range total, and request count always remain whenever it is shown.
