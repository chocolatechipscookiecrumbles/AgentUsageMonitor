...
| [8. Detect an external Codex login while disconnected](follow-ups.md#8-detect-an-external-codex-login-while-disconnected) | Bug fix — **Queued** | Implement the dedicated 30-second/activation [external-login detection plan](../superpowers/plans/2026-07-17-external-codex-login-detection.md); retain controller ownership, one authentication refresh, custom `CODEX_HOME`, teardown, credential privacy, and no menu-owned watcher or quota scheduler. |
...
| External Codex CLI login is not detected from the disconnected stage | **Queued** | Implement controller-owned read-only rechecks immediately on activation and at most every 30 seconds while disconnected; cancel after connection and trigger exactly one authentication refresh. | [external-login plan](../superpowers/plans/2026-07-17-external-codex-login-detection.md), [CodexConnectionController.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Connection/CodexConnectionController.swift), [CodexConnectionService.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Connection/CodexConnectionService.swift) |
...
- [External Codex Login Detection](../superpowers/plans/2026-07-17-external-codex-login-detection.md) — queued controller-owned reconciliation for an independently completed Codex login.
