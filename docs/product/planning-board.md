...
| [2. Automatic recovery from interrupted sign-in](follow-ups.md#2-automatic-recovery-from-interrupted-sign-in-flow) | Bug fix — **Needs plan** | Recover cancelled, failed, or timed-out Browser/CLI attempts to a usable disconnected state with clear status; restore both sign-in actions automatically and support repeated attempts without relaunch. |
...
| [5. Simplify the General Settings context rail](follow-ups.md#5-simplify-general-settings-context-rail) | Feature — **Needs plan** | Remove **Current Label**, remove **Current Scope**, and remove the duplicate General-page Preview; make one larger, full-width context-rail menu-bar preview the authoritative preview. |
...
| [8. Detect an external Codex login while disconnected](follow-ups.md#8-detect-an-external-codex-login-while-disconnected) | Bug fix — **Needs plan** | Let `CodexConnectionController` detect an independently completed login within a documented bound, coalesce checks, publish connected state, and trigger exactly one authentication refresh; stop detection after connection, preserve custom `CODEX_HOME` and teardown behavior, never read credential files, and do not let menu presentation create duplicate watchers or schedulers. |
...
| Stable Settings regions and shared native switches | **Deferred** | Split from the recovered multi-agent plan and revalidate current global-sidebar geometry before implementation. | [provider follow-ups Task 1](../superpowers/plans/2026-07-14-settings-provider-followups.md#task-1-stabilize-settings-region-geometry-and-switch-styling), [SettingsView.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Settings/SettingsView.swift) |
...
| Simplified General context rail | **Needs plan** | Keep one larger full-width menu-bar preview; remove Current Label, Current Scope, and the duplicate General-page Preview; verify both rail states. | [product follow-up 5](follow-ups.md#5-simplify-general-settings-context-rail), [GeneralSettingsContextView.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Settings/GeneralSettingsContextView.swift), [GeneralSettingsView.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Settings/GeneralSettingsView.swift) |
...
| External Codex CLI login is not detected from the disconnected stage | **Needs plan** | Add controller-owned, bounded status rechecks for an externally changed Provider Session; cancel them after connection and trigger exactly one authentication refresh without reading credentials. | [product follow-up 8](follow-ups.md#8-detect-an-external-codex-login-while-disconnected), [CodexConnectionController.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Connection/CodexConnectionController.swift), [CodexConnectionService.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Connection/CodexConnectionService.swift) |
| Refresh-interruption notification can be lost after failed submission | **Queued** | Re-offer the same stable episode event until delivery succeeds; do not add another counter or change the key. | [known limitation](../superpowers/plans/2026-07-14-disconnection-notification-backoff.md#authentication-classification-correction--2026-07-14), [QuotaNotifier.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Notifications/QuotaNotifier.swift), [QuotaMonitor.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Monitoring/QuotaMonitor.swift) |
| Interrupted browser/CLI sign-in can remain stuck | **Needs plan** | Add evidence-based cancellation/failure/timeout recovery and prove repeated sign-in attempts work without relaunch. | [product follow-up 2](follow-ups.md#2-automatic-recovery-from-interrupted-sign-in-flow), [CodexConnectionController.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Connection/CodexConnectionController.swift), [CodexConnectionService.swift](../../CodexUsageMonitor/Sources/CodexUsageMonitor/Connection/CodexConnectionService.swift) |
