...
- External-login follow-up implemented on 2026-07-17: `CodexConnectionController` now owns silent `account/read` rechecks immediately after application activation and at most every 30 seconds only while state is `.disconnected`. An isolated user acceptance run observed independent `codex login` reconnect through both paths without an in-app sign-in action. The remaining one-refresh count, repeated-event coalescing, negative, logout, sleep/wake, and teardown checks remain unobserved in the [External Codex Login Detection Implementation Plan](2026-07-17-external-codex-login-detection.md). It neither reads credentials nor adds a quota scheduler or native-menu timer.
