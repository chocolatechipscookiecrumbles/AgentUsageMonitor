...
- Queued follow-up recorded on 2026-07-17: a user can remain on the disconnected stage after completing `codex login` independently, without first selecting either app sign-in action. `start()` reads status once; the bounded CLI watcher is created only by `signInWithCLI()`; and the later silent recheck is coupled to a failed quota refresh rather than to a newly valid external Provider Session. Runtime reproduction was not manufactured against the user's active account. The [External Codex Login Detection Implementation Plan](2026-07-17-external-codex-login-detection.md) defines the privacy-safe controller-owned 30-second and activation-triggered acceptance boundary.
