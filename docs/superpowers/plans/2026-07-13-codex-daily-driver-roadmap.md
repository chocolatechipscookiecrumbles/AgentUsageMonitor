...
| Menu-popover token activity card (formerly Dashboard) | Feasible and queued by user direction on 2026-07-28 | Implement the revised [token activity card plan](2026-07-14-dashboard.md): reconcile local records, graph 30-minute activity, then show fixed detail rows; no separate window or page |
...
External-login follow-up implemented on 2026-07-17: while explicitly disconnected, the connection controller reads the existing provider-owned `account/read` status immediately when the app becomes active and at most every 30 seconds. An isolated user-owned acceptance run observed independent `codex login` reconnect within the bound without an in-app action, then reconnect promptly after activation. The remaining exact authentication-refresh count, repeated-event coalescing, negative, logout, sleep/wake, and teardown checks remain recorded as unobserved in the [External Codex Login Detection Implementation Plan](2026-07-17-external-codex-login-detection.md). The change does not read credential files, add a quota scheduler, or add a native-menu timer.
