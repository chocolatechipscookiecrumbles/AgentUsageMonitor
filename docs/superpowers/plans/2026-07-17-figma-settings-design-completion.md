...
**Status:** **Implementation complete — Tasks 1–6 are implemented.** The user directly inspected the card/rail destination matrix. The separately recorded destination-switch text-compositing defect remains **Deferred** by user direction after two rejected workarounds; it requires a dedicated prototype and signed-app frame capture before another repair. This plan does not revive or merge `feature/figma-settings-port`.
