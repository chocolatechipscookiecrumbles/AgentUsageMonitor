...
Native `ProgressView.tint` can apply an appearance-dependent control treatment that visibly darkens the requested provider color. `ProviderQuotaProgressBar` is the required provider-page bar template: a palette-derived track plus a `Capsule` filled directly with its supplied provider tint. Do not add an unrelated blend mode, overlay, or native progress style. `AgentQuotaWindowKind` supplies the only allowed foreground opacity: five-hour is `1`, weekly is `0.72`. This preserves the exact #576DFF Codex five-hour color while deliberately letting the weekly sibling composite with the Light/Dark card surface.
...
`AgentQuotaWindowKind` owns the two horizon treatments. The **five-hour** bar uses the exact, fully opaque provider tint (Codex #576DFF). The **weekly** bar uses that same provider tint at the shared `0.72` foreground opacity, allowing the current Light/Dark card surface to produce the more subdued sibling shade without introducing a second hue. Both reset readouts include abbreviated date, year, and time. The adaptive track remains separate.
