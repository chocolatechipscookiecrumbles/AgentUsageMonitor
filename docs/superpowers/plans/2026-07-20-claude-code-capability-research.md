...
**Status:** Gate satisfied as of 2026-07-23. The real Claude provider page, connection controller, refresh cycle, explicit unavailable state, and shared-pool copy are implemented. OAuth credential reuse remains authorized only for this personal, undistributed build; statusLine remains the zero-cost fallback; the narrow local-JSONL reader remains historical-only. Claude notifications are still separately deferred.
...
3. ~~The product copy correctly scopes what the number means (Claude Code only vs. shared with Claude chat/Cowork) based on direct evidence for the plan type in use, not assumption~~ — **satisfied 2026-07-23.** Anthropic's Pro/Max-specific [Use Claude Code with your Pro or Max plan](https://support.claude.com/en/articles/11145838-use-claude-code-with-your-pro-or-max-plan) states that both plans share usage limits across Claude and Claude Code and that activity in both products counts against the same limits. This directly supports the shipped wording, "Weekly usage is shared with Claude chat, not Claude Code alone." `ClaudeUsageDisplayModel.weeklyScopeCaveat` appears both under the weekly quota row and in Data & Privacy, so the number is never shown without its scope.
4. ~~A single owner exists for the read cycle (mirroring `QuotaMonitor`'s ownership for Codex), with explicit teardown and no second polling source~~ — **satisfied 2026-07-20** by `ClaudeUsageMonitor` (see [usage monitor owner plan](2026-07-20-claude-usage-monitor-owner.md)), a `@MainActor` `ObservableObject` with explicit `start()`/`stop()` and `deinit`-cancelled polling, plus a tested, non-destructive `ClaudeStatusLineInstaller` for setup. Note the state model is deliberately simpler than `AgentConnectionState`/`ConfirmationState`: because the bridge only ever produces a last-known snapshot, there is no "live/confirmed" state to distinguish, only available-or-not. **Reachability update 2026-07-23:** `Scripts/build-app.sh` now bundles `ClaudeUsageBridge/`; the installer copies it from the signed app resource to the deterministic app-owned Application Support path before use.
5. ~~If no zero-cost source exists, the fallback is an explicit, permanent "not available" state — not a best-effort estimate presented as a quota~~ — **satisfied 2026-07-23.** `ClaudeAgentSettingsView` renders "Not available" when there is no reading, `AgentQuotaWindowRow` reserves both windows and shows an explicit unavailable/idle state rather than removing a row, a cached result is labelled with its source and age ("Cached Claude OAuth result · 3 hours ago") rather than passed off as live, and `ProviderContextSummaryTests` pins that a disconnected provider renders placeholders and never `0%`. A genuine first run now uses the centered setup card after initial source discovery; durable history and safe legacy cache/statusLine evidence keep later lapses on the factual recovery page. `ClaudeCodePreviewSettingsView` is deleted: it was unreachable after routing moved to the real page, and its copy claimed the app reads no Claude credentials or usage, which is no longer true.

**Status 2026-07-23.** All five criteria are satisfied. Claude is no longer a static preview — it is a `.supported` catalog entry with a real page, a rail card, a user-reachable refresh, an explicit first-run setup state, and a bundled statusLine bridge resource. There are still **no Claude notifications** (the Usage Warnings section was deliberately deferred), and the one-click statusLine configuration/merge UX remains deferred even though the installer's production bridge path is now resolvable.

**Credential-method update — 2026-07-21, superseded 2026-07-23.** The earlier plan proposed **two co-equal, user-selectable credential methods**, mirroring Codex's browser/CLI sign-in pair (see [credential methods plan](2026-07-21-claude-oauth-web-login-provider.md)):
...
The current shipped decision is narrower: browser/setup-token sign-in remains **shelved as unverified and is not presented as working**. The page exposes the explicit Claude Code Keychain-credential action plus passive status-line capture. `ClaudeConnectionController` is wired into the supported Claude page for the credential action; criterion 3/5 UI work is complete. A self-run PKCE flow remains deliberately unshipped because it would reuse Claude Code's `client_id`; see [spike findings](2026-07-21-claude-oauth-web-login-spike-findings.md).
...
- [Use Claude Code with your Pro or Max plan — Claude Help Center](https://support.claude.com/en/articles/11145838-use-claude-code-with-your-pro-or-max-plan) — direct Pro/Max evidence that Claude and Claude Code draw from the same usage limits.
