...
- [x] **Step 1b (probe — done):** Evidence gathered in [claude-usage-endpoint-rate-safety.md](../../development/claude-usage-endpoint-rate-safety.md). The real endpoint risk was **not** the interval but a **missing `User-Agent: claude-code/<version>` header**, which the community found puts callers in an aggressive 429 bucket; our request now sends it (plus `Content-Type`). A 429 is now obeyed via `Retry-After` back-off in the collector. The **5-minute floor is kept** (12 reads/hour, negligible). A controlled live probe was not run (no sandbox access to the token); the app now self-corrects on any 429, so a wrong interval assumption cannot compound the limit.
