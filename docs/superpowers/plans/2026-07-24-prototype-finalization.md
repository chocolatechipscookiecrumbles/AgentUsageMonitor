...
- [x] **Step 2 (done):** `AgentUsageWarningsSection` restyled to the A1 chip mockup (header/label/helper copy, blue selected + checkmark, gray unselected), wired into the Claude page, both pages repointed to the provider-scoped store. Removed the global "Remaining Quota" section from `NotificationSettingsView`; kept the master toggle and Other Warnings. `NotificationSettingsContextView` preview now reads across providers. (Hover/pressed states use the default plain-button behavior; not separately styled.)
