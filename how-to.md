...
The current interface uses native SwiftUI. A separate Dashboard page or window is no longer planned. The **Token Monitor** card is implemented in both provider tabs of the menu popover: while the app runs it automatically builds a reconciled index from field-scoped provider records, with no network call and no token cost. Recent totals are cached in the app's own folder, so a launch shows the previous session's figures immediately and refreshes them in the background rather than rebuilding from nothing. The dynamic-height **This Mac · observed** card uses hoverable 30-minute completion-time bars and lists provider-native token categories, Requests, the top three short model groups plus counted Other, and Last Request across all observed dates. Compact loading/missing/unsafe states expand when a readable current day is available. The card is independent of quota availability and does not change quota refreshes, notifications, authentication, or provider limits.

Each agent's Settings page has a **Token Monitor** section: one **Show token monitor** toggle plus **Activity chart**, **Token categories**, **Model usage**, and **Last request**. Turning an agent's Token Monitor off stops reading that agent's records and removes what was cached for it, so it is a collection control rather than a display filter. The card's title, **This Mac · observed**, day total, and request count always remain whenever it is shown.
