...
Open **Settings…** from the menu popover (or press `Command-,`) to use the separate Settings window. Its General, Refresh, Agents, Data & Privacy, and Diagnostics tabs show current app, collection, integration, retention, and classified-outcome status; Notifications contains the working preferences. Agents identifies OpenAI Codex as the only active integration and lists Claude Code and GitHub Copilot as planned and not connected. The Refresh tab also provides a real **Refresh now** action. Interval choices, Codex sign-in, launch at login, export, and deletion remain in later roadmap branches rather than appearing as nonfunctional controls.
