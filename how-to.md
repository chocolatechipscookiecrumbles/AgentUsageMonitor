...
- The bottom action row (**Refresh Now**, **Notification Settings**, **Preferences…**, **Quit Codex Usage Monitor**) remains available.
...
If Codex is signed out elsewhere while the monitor remains open, the next unavailable quota refresh rechecks `account/read` and moves the Codex tab to the disconnected stage without requiring an app relaunch.
...
Data & Privacy also acts on that storage. **Reveal in Finder** and **Copy Path** open or copy the Application Support folder, falling back to its parent before the first refresh writes anything. **Export Local Data…** writes every listed store to one JSON file you choose, naming and marking as unavailable any store this Mac has not written rather than omitting it; nothing outside the app's own folder is included, so Claude Code's Keychain item and the agents' own records are never exported. The exported file leaves the app's owner-only folder and is protected only by where you save it. Deletion controls remain deferred — remove the files in Finder to clear this data today.

Diagnostics offers three actions. **Copy Report** copies exactly what the page shows as plain text, carrying the same stable classifications and no raw provider text or quota values. **Reveal in Finder** selects `refresh-diagnostics.json`. **Clear History…** asks for confirmation, then deletes up to 30 days of recorded outcomes and classified failures; quota readings and the refresh schedule are unaffected and the next completed refresh starts a new history.

The current interface uses native SwiftUI. A separate Dashboard page or window is no longer planned. The **Token Monitor** card is implemented in both provider tabs of the menu popover: while the app runs it automatically builds a reconciled index from field-scoped provider records, with no network call and no token cost. Recent totals are cached in the app's own folder, so a launch shows the previous session's figures immediately and refreshes them in the background rather than rebuilding from nothing. The dynamic-height card names its range under the title — **Today** or **This week** — and lists provider-native token categories, Requests, the top three short model groups plus counted Other, and Last Request across all observed dates. Everything it shows is observed on this Mac and is never account, billing, or quota usage. Compact loading/missing/unsafe states expand when a readable range is available. The card is independent of quota availability and does not change quota refreshes, notifications, authentication, or provider limits.

Each agent's Settings page has a **Token Monitor** section: one **Show token monitor** toggle, a **Range** choice of **Day** or **Week**, and the **Activity chart**, **Token categories**, **Model usage**, and **Last request** section toggles. Turning an agent's Token Monitor off stops reading that agent's records and removes what was cached for it, so it is a collection control rather than a display filter. The card's title, range line, range total, and request count always remain whenever it is shown.
