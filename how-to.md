...
The menu-bar content uses native menu presentation. **Refresh now**, **Settings…**, **Open Notification Settings…**, and **Quit Codex Usage Monitor** remain inline menu commands. Its refresh row shows the last attempt beside a live next-refresh countdown. **Confirmed / completed** means the latest attempt returned trusted live data. **Cached / paused** means it did not: the app retains and labels the last confirmed result when one exists, shows the last successful and attempted times, and never presents an unconfirmed result as current. When notifications are disabled, a concise standalone status row appears without overlapping refresh metadata or later commands.
