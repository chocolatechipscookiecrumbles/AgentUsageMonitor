...
Open **Settings…** from the menu popover (or press `Command-,`) to use the separate Settings window. Its General, Refresh, Agents, Data & Privacy, and Diagnostics tabs show current app, collection, integration, retention, and classified-outcome status; Notifications contains the working preferences. General contains the working menu-bar Appearance and Show controls plus a live preview. Changing those controls updates the menu label immediately. Quota values update after each launch, wake, authentication, manual, or scheduled refresh; the maximum scheduled delay follows the frequency selected in Refresh (60, 90, 120, 300, or 600 seconds, while Automatic may temporarily use 30 seconds). Agents uses a left sidebar for OpenAI Codex, Claude Code, and GitHub Copilot, with a separate detail pane inside the tab for each. Codex is the only active integration and its pane shows the same connection state and sign-in actions as the menu; Claude Code and GitHub Copilot are planned and not connected. Refresh contains the working frequency picker, effective-policy explanation, timestamps, and **Refresh now**. Launch at Login, System/Light/Dark appearance, export, and deletion remain in later roadmap branches rather than appearing as nonfunctional controls.
