...
The current interface uses native SwiftUI. A separate Dashboard page or window is no longer planned. The **Token activity** card is implemented in both provider tabs of the menu popover: while the app runs it automatically builds an in-memory-only index from field-scoped provider records, with no network call and no token cost. The dynamic-height **This Mac · observed** card uses hoverable 30-minute completion-time bars and lists provider-native token categories, Requests, the top three short model groups plus counted Other, and Last Request across all observed dates. Compact loading/missing/unsafe states expand when a readable current day is available. The card is independent of quota availability and does not change quota refreshes, notifications, authentication, or provider limits.

Reading is automatic and needs no setup: if you use Codex or Claude Code on this Mac, activity appears on its own. Nothing is written — quitting the app discards the index, and the next launch rebuilds it. Records that cannot be read safely are shown as unavailable rather than counted as zero, so an empty day and an unreadable one never look alike.

**Known limitation before release:** the card's tallest state makes the Codex tab about 982 points tall, which exceeds the usable height of a typical laptop screen. The popover is deliberately non-scrolling, so this needs a layout decision before the card ships enabled by default. See the implementation plan for the measurements.
