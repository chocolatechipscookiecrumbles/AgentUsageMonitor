...
The monitor refreshes at launch, after wake, on **Refresh now**, and at the selected foreground interval while its process is running. The Refresh tab offers Automatic, 1 minute, 1 minute 30 seconds, 2 minutes (the default), 5 minutes, and 10 minutes. **Refresh on wake** is shown as always on because the monitor already refreshes after macOS wakes; **Refresh on open** is visibly planned and has no effect yet. Automatic considers confirmed usage rate, remaining quota, reset proximity, forecast confidence, and recent failures. Only Automatic may temporarily use 30 seconds near a warning threshold, qualified exhaustion, or reset verification; that burst ends after ten minutes, after its trigger passes, or after unsuccessful live reads. After three consecutive unsuccessful refreshes, every mode temporarily changes to a 10-minute retry cadence. A confirmed update automatically restores the selected healthy-state schedule. Only one collection runs at a time, and the manual button is disabled during an active refresh.

Open **Settings…** from the menu popover (or press `Command-,`) to use the separate Settings window. Its global `SettingsNavigationSidebar` selects General, Notifications, Refresh, Agents, Data & Privacy, and Diagnostics. Destination changes use one non-animated presentation transaction, including future sidebar destinations, so a page switch does not create transient text layers. It opens with the Context Rail hidden at 680 × 560 points. Use the page-header Context Rail button to show the 210-point rail; the window adds 211 points only at its right edge and the sidebar and Settings Page retain their width. Settings cards use consistent compact rows, dividers, native switches, and leading control descriptions. General groups **Launch at login** and **Enable keyboard shortcuts** together, then presents **Style**, **Show**, and Settings-window **Appearance** as bounded native segments. Appearance changes the Settings presentation only; the native menu remains controlled by macOS. Its one menu-bar preview is in the Context Rail; there is no page-level duplicate Preview, Current Label, or Current Scope. Diagnostics owns the app **Name**, **Version**, and **Build** rows. Launch at Login reflects the real macOS Login Items state and links to System Settings when approval is required. Disabling keyboard shortcuts removes the app-local `Command-R` binding while leaving both **Refresh now** buttons and the standard Settings/Quit commands available. The user directly inspected the six destinations with both rail states, Light/Dark, relevant conditional states, scrolling, keyboard, VoiceOver, focus preservation, and native-menu appearance; the new destination-switch regression and separate System-appearance transition matrix remain tracked acceptance work. Menu-bar controls update the menu label immediately. Quota values update after each launch, wake, authentication, manual, or scheduled refresh; the maximum scheduled delay follows the frequency selected in Refresh (60, 90, 120, 300, or 600 seconds, while Automatic may temporarily use 30 seconds). Agents is one grouped, scrollable Settings page for OpenAI Codex, Claude Code, and GitHub Copilot. Codex is the only active integration and its section shows the same connection state and sign-in actions as the menu; Claude Code and GitHub Copilot are planned and not connected. Refresh contains the working interval selector, presentational wake/open rows, effective-policy explanation, timestamps, and **Refresh now**. Export and deletion remain later roadmap work.
