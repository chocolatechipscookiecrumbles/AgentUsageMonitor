...
* **Historical (superseded Settings Foundation description):** `feature/settings-foundation` originally added General, Refresh, Agents, Data & Privacy, and Diagnostics around Notifications with lower-content sidebars. The current product uses the global Settings sidebar and a single grouped, scrollable Agents page; Codex is current while Claude Code and GitHub Copilot remain visibly planned and not connected.
* `feature/adaptive-refresh` is implemented. It replaces the repeating five-minute timer with persisted fixed/automatic modes, a one-shot scheduler, an event-driven native-menu timing row, and monitor-owned confirmed/completed or cached/paused display state. The July 17 repair removes the per-second observed child and shows an absolute next-refresh time, preserving semantic refresh transitions without mutating native-menu geometry every second. Signed connected/scheduled presentation and the original pointer/scroll highlight regression passed direct inspection; Settings mode switching, a natural cached/paused state, and unmanufactured conditional menu states remain targeted checks. A future true countdown is deferred behind an ADR and fixed-geometry signed prototype.
* `feature/codex-connection` is implemented and user-accepted for the in-app Browser and visible CLI flows. Those paths, disconnected presentation, and return to quota refresh work without reading credentials; acceptance is not repeated by signing out the user's active account. A newly reported limitation remains planned: an independent `codex login` performed while the app is already disconnected is not guaranteed to trigger connection-state reconciliation because the bounded watcher starts only from the app-owned CLI action.
...
* The native Figma Settings completion implementation preserves the global sidebar and current semantic theme. New Settings windows start with a hidden 680 × 560 Context Rail; showing it adds 211 points only to the right edge. General now has one full-width Context Rail menu-bar preview with no Current Label, Current Scope, or duplicate page Preview; its Menu Bar Icon group contains Style, Show, and Settings-window System/Light/Dark Appearance segments, while Diagnostics owns Name, Version, and Build. Appearance does not recolor the native menu, which remains macOS-controlled, and current independent Boolean preferences use native switches. Package, signed-bundle, signature, plist, and focused geometry-regression checks pass; direct six-destination rail, segmented-control, Diagnostics, VoiceOver, conditional-state, and appearance interaction acceptance remains manual. The historical Figma branch and local `High-fidelity macOS menu UI v4` artifact remain structural references only, with no React runtime or assets imported.
* The same Settings follow-up branch adds General controls for Launch at Login, System/Light/Dark appearance, and app-local keyboard-shortcut enablement. The July 16 fix keeps one Settings presentation owner and resolves System from live macOS effective appearance, eliminating the reproduced mixed window without recoloring the native menu. Signed-app acceptance covers both macOS host appearances, live OS switching, the reciprocal native-menu boundary, and all six destinations in explicit Light; manufactured conditional states remain recorded in `2026-07-15-settings-system-appearance-transition.md`. Launch at Login acceptance was skipped for this pass. Initial custom shortcut scope is Command-R for Refresh; standard Command-, and Command-Q remain available.
...
* **Historical (superseded Settings Foundation description):** the former top-tab/inner-provider-sidebar shell is retained only as branch history. The current product uses the global Settings sidebar and a single grouped, scrollable Agents Settings page described above.
* The historical stacked `feature/adaptive-refresh` work is merged. Its current native-menu timing behavior and remaining acceptance are recorded above and in `docs/superpowers/plans/2026-07-14-native-menu-refresh-row.md`.
...
* **Historical (superseded Agents-shell description):** the former full-width top Settings tab bar and inner 180-point provider sidebar no longer describe the current product. The global Settings sidebar selects Agents, whose grouped page retains Codex connection actions while Claude Code and GitHub Copilot remain planned.
...
* The former separate Dashboard-window plan was replaced on 2026-07-28 by a **Token Monitor** card in the existing Codex and Claude menu-popover tabs, now implemented. Each agent's Settings page controls whether its card appears and which of its four sections are shown; hiding an agent's card also stops reading that agent's records. While the app runs, automatic field-scoped reads build a reconciled index, and recent totals are cached across launches so a new instance renders before it rescans. The dynamic-height **This Mac · observed** card uses hoverable 30-minute completion-time bars and lists provider-native token rows, Requests, the top three Short Model Name groups plus counted Other, and Last Request across all observed dates. It distinguishes missing records, unsafe reads, and a valid zero day, remains independent of provider quota, and adds no model turn, network read, separate page, range picker, or popover timer. Its tallest state exceeds the usable height of a typical laptop screen in the non-scrolling popover; that layout decision is open.
